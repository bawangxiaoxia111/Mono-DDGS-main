...
@torch.no_grad()
def render_training_image_two(scene, gaussians, viewpoints, render_func, pipe, background, stage, iteration, time_now,
                          dataset_type, flag):
    def render(gaussians, viewpoint, path, scaling, cam_type, flag):
        # scaling_copy = gaussians._scaling
        render_pkg = render_func(viewpoint, gaussians, pipe, background, stage=stage, cam_type=cam_type, flag=flag)
