...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask, image_name, uid,
        ...
        # print("the shape of world_view_transorm: ", self.world_view_transform.size()) # [4, 4]
        # .cuda()
        self.projection_matrix = getProjectionMatrix(znear=self.znear, zfar=self.zfar, fovX=self.FoVx, fovY=self.FoVy).transpose(0,1)
        # print("the shape of projection_matrix: ", self.projection_matrix.size()) # [4, 4]
        # .cuda()
        self.full_proj_transform = (self.world_view_transform.unsqueeze(0).bmm(self.projection_matrix.unsqueeze(0))).squeeze(0)
        # print("the shape of full_proj_transform: ", self.full_proj_transform.size()) # [4, 4]
        self.camera_center = self.world_view_transform.inverse()[3, :3]
        # print("the shape of camera_center: ", self.camera_center.size()) # [3]
