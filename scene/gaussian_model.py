...
class GaussianModel:
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': list(self._deformation.get_dense_parameters()), 'lr': training_args.grid_lr_init * self.spatial_lr_scale, "name": "dense"},
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, density_threshold=20, displacement_scale=20,
        ...
        selected_pts_mask = torch.logical_and(grads_accum_mask, torch.max(self.get_scaling,
                                                                          dim=1).values <= self.percent_dense * scene_extent)
    ...
    def _plane_regulation_dense(self):
        multi_res_grids = self._deformation.deformation_net.dense.denses
        total = 0
    ...
    def _l1_regulation(self):
        ...
        for grids in multi_res_grids:
            if len(grids) == 3:
                ...
                spatiotemporal_grids = [1, 2, 4, 5, 7, 8]
            for grid_id in spatiotemporal_grids:
                total += torch.abs(1 - grids[grid_id]).mean()
        return total

    def compute_regulation(self, time_smoothness_weight, l1_time_planes_weight, plane_tv_weight):
        return plane_tv_weight * self._plane_regulation() + plane_tv_weight * self._plane_regulation_dense() + \
               time_smoothness_weight * self._time_regulation() + l1_time_planes_weight * self._l1_regulation()
